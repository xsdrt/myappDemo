...
var (
	...
	user     = "postgres"
	password = "secret"
	dbName   = "lowspeed_test"
	port     = "5435"
	dsn      = "host=%s port=%s user=%s password=%s dbname=%s sslmode=disable timezone=UTC connect_timeout=5" //data source name to connect to the database..
)
...
var authData = []struct { //variable for a table test...
	name        string
	token       string
	email       string
	errExpected bool
	...
	{"invalid", "abcdefghijklmnopqrstuvwxyz", "not@here.com", true, "invalid token accepted as valid"},
	{"invalid_length", "abcdefghijklmnopqrstuvwxy", "not@here.com", true, "token of wrong length token accepted as valid"},
	{"no_user", "abcdefghijklmnopqrstuvwxyz", "not@here.com", true, "no yser, but token accepted as valid"},
	{"valid", "", "mia@idk.com", false, "valid token reported as invalid"},
}
